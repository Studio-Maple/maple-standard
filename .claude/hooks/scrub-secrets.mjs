...
const PATTERNS = [
  ...
  // candidate to the end of the current line — on one long line (a
  // minified blob, a huge log line with no newlines) that's O(n) per
  // candidate, ~O(n^2) overall; reviewer measured ~1s/100KB against this
  // hook's 5s timeout. Two fixes, applied together: (1) `requireSubstring`
  // below skips this pattern entirely when "vercel" doesn't appear
  // anywhere in the text — the common case, one cheap indexOf instead of a
  // regex pass; (2) the lookahead itself is now bounded to 200 chars
  // instead of the rest of the line, so even a text that DOES contain
  // "vercel" somewhere can't force a full-remaining-line rescan per
  // candidate.
