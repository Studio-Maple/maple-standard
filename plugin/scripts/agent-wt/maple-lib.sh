...
# worktree + ephemeral `<branchPrefix><slug>` branch; the ONLY path back to
# the target branch is `maple-land`, which holds a GLOBAL lock while it
# rebase -> gate -> pushes. The lock serializes both the git integration AND
# the gate command, so two sessions can never collide on the tree, the
# branch, or the gate.
#
# maple.config.json keys this file reads (all optional — sane defaults below):
#   worktree.remote                  default "origin"
#   worktree.targetBranch            default: origin/HEAD's branch, else "main"
#   worktree.branchPrefix            default "agent/"
#   worktree.root                    default "../<repo-name>-wt" (sibling dir)
#   worktree.nodeModulesDirs         default ["."]  (dirs to junction-link node_modules from)
#   worktree.envFiles                default []     (gitignored env files to hardlink)
#   worktree.freshDepsCommand        default "npm ci"
#   worktree.lock.ttlSeconds         default 1800  (30min — stale-lock steal threshold)
#   worktree.lock.waitSeconds        default 3600  (60min — total wait before giving up)
#   worktree.lock.pollSeconds        default 5
...
MAPLE_BRANCH_PREFIX="${MAPLE_BRANCH_PREFIX:-$(maple_cfg worktree.branchPrefix 'agent/')}"
MAPLE_REMOTE="${MAPLE_REMOTE:-$(maple_cfg worktree.remote origin)}"

_maple_default_target_branch() {
  local cfgVal; cfgVal="$(maple_cfg worktree.targetBranch '')"
  if [ -n "$cfgVal" ]; then printf '%s' "$cfgVal"; return; fi
  # No config — try to read the remote's default branch; fall back to "main".
  local head
...
_maple_default_wt_root() {
  local cfgVal; cfgVal="$(maple_cfg worktree.root '')"
  if [ -n "$cfgVal" ]; then
    case "$cfgVal" in
...
MAPLE_LOCK_TTL="${MAPLE_LOCK_TTL:-$(maple_cfg worktree.lock.ttlSeconds 1800)}"
MAPLE_LOCK_WAIT="${MAPLE_LOCK_WAIT:-$(maple_cfg worktree.lock.waitSeconds 3600)}"
MAPLE_LOCK_POLL="${MAPLE_LOCK_POLL:-$(maple_cfg worktree.lock.pollSeconds 5)}"
...
maple_branch_for() { printf '%s%s' "$MAPLE_BRANCH_PREFIX" "$1"; }
...
# maple.config.json worktree.nodeModulesDirs — dirs (relative to repo root)
...
maple_link_node_modules() {
  ...
  done < <(maple_cfg_array worktree.nodeModulesDirs '.')
}

# maple.config.json worktree.envFiles — gitignored env files (relative to
...
maple_link_env_files() {
  ...
  done < <(maple_cfg_array worktree.envFiles '')
}
