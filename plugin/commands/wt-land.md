...
the only sanctioned path to the target branch. Under one global lock it
rebases onto the freshest remote target, runs the configured gate command
for the chosen tier, fast-forward-pushes, and prunes the worktree. A red
gate / rebase conflict / non-ff push makes it **refuse and release** —
nothing un-gated lands.
