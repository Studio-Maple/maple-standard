#!/usr/bin/env node
/**
 * next-task-id.mjs — collision-free ID allocator for #T tasks and D decisions.
 *
 * Parallel sessions (or parallel agents) guessing "the next number" causes
 * real collisions. Allocate from here instead. Paths follow this template's
 * flat docs/ layout (docs/tasks.md, docs/decisions.md, docs/log.md).
 *
 *   node scripts/next-task-id.mjs              -> next free #T id + any #T collisions
 *   node scripts/next-task-id.mjs --decision   -> next free D id (decisions.md)
 *   node scripts/next-task-id.mjs --session     -> next free S id (log.md)
 *   node scripts/next-task-id.mjs --check       -> exit 1 if any #T collision (gate use)
 *
 *   node scripts/next-task-id.mjs --add --section "Inbox" --title "…" --body "…"
 *       -> allocate the next #T, insert `- [ ] **#T### — <title>.** <body>` at the
 *          TOP of the named tasks.md section, print the allocated id.
 *   node scripts/next-task-id.mjs --add --decision --title "…" --body "…"
 *       -> allocate the next D, insert `## D### | YYYY-MM-DD | <title>` + body at the
 *          TOP of decisions.md (newest-first, right after the preamble), print the id.
 *   node scripts/next-task-id.mjs --add --session --title "…" --body "…"
 *       -> allocate the next S, prepend `## S### | YYYY-MM-DD | <title>` + body at the
 *          TOP of log.md (newest-first session log), print the id.
 *
 ...
 *
 * docs: docs/decisions.md · docs/tasks.md · docs/log.md
 */
...
/** Resolve the doc paths, honouring the test-only env overrides. */
export function resolvePaths(env = process.env) {
  return {
    tasks: env.NEXT_TASK_ID_TASKS_FILE || join(ROOT, "docs", "tasks.md"),
    decisions: env.NEXT_TASK_ID_DECISIONS_FILE || join(ROOT, "docs", "decisions.md"),
    log: env.NEXT_TASK_ID_LOG_FILE || join(ROOT, "docs", "log.md"),
  };
}
  ...
  if (kind === "task") {
    ...
    if (!findSectionHeader(tasksTxt, section)) {
      throw new AllocError(
        `section "${section}" not found in tasks.md. Existing sections: ${listSections(tasksTxt).join(", ")}`
      );
    }
  }
...
export function insertTaskEntry(tasksTxt, section, entryLine) {
  ...
  if (!m) throw new AllocError(`section "${section}" not found in tasks.md`);
...
async function dispatch(argv, { env = process.env } = {}) {
  ...
  if (opts.add) {
    ...
    const where = kind === "decision" ? "docs/decisions.md" : kind === "session" ? "docs/log.md" : `"${opts.section}" in docs/tasks.md`;
