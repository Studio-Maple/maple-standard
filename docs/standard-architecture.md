...
| **maple-standard template repo** (unchanged) | `C:/Projects/Maple-Standard` (D056 — top-level, no longer nested under Studio-Maple), clone-and-rename | Full Next.js+Supabase+Vercel scaffold, its own CI workflows, its own `.claude/hooks/*` including the JS/TS-specific ones | Greenfield-only concern. A brand-new project still gets the fastest path: clone, rename, done — it doesn't need a bootstrap command because there's no existing state to reconcile. |
